...
trigger1 = var(1) = 1 && !var(33) && roundstate = 2 && p2dist x > 0 && power >= 500 && var(20) <= 60
...
triggerall = AIlevel && numenemy
triggerall = !var(33) && roundstate = 2 && statetype != A && power >= 2000 && power < 3000
triggerall = !(enemynear, ctrl) && (p2stateno != [120, 155]) && p2statetype != L
...
type = changestate
;value = ifelse(power>=2000,ceil(ifelse(random<300,3600,3500)),3200)
value = ifelse(power>=2000,3500,3200)
...
[State -1, AI Hisshou/Shissou Buraiken]
type = changestate
value = ceil(ifelse((p2dist x =[69,80]),3600,ifelse(random<300,3600,3500)))
triggerall = AIlevel && numenemy && !var(20)
...
trigger1 = ctrl
trigger2 = stateno = 3300 && ((anim = [3304,3320])||anim=3301)
...
type = changestate
value = ifelse(statetype=A,1502,ifelse(statetype=C,1501,1500)) +ifelse((var(1) != 2 && power >= 500 && power < 3000 && random < 200),10,0)
